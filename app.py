...
def create_app():
    ...
    def sglistsingle():
        ...
        def bpricegen(x):
            ...
            if ownlist == "N":
                x_facevalue = 0 if x["FACE_VALUE"] == "NA" else x["FACE_VALUE"]
            ...
            if own == "Y":
                ...
                if lowest_comp_price != "NO COMP":
                    ...
                    # positive difference == lower bprice
                        ...
                        # SO THE TEAM WHEN THEY SEE BFLOOR WILL COMPARE TO THE 2ND LOWEST AND -1
                        # TO THEIR APROX BPRICE THEREFORE AVOIDING ANY WEIRD BPRICE FROM
        ...
        if team == "Reds" or team == "Cubs" or team == "Guardians" or team == "Royals":
    ...
    def sgreport():
        ...
        def matchfunction(x):
            ...
            sgseatmin = x["sgminseat"]
            sgseatmax = x["sgmaxseat"]
            sgseatmin = int(sgseatmin) if sgseatmin != "NA" else 0
            sgseatmax = int(sgseatmax) if sgseatmax != "NA" else 0
            sbrow = x["row_match"]
            sgrow = x["ROW_NAME"]
            x["FACE_VALUE"]
            float(x["listprice"])
    ...
    def trtmlbcounts():
        ...
        def dynamic(x):
            ...
            if (minprice == 0) & (maxprice == 0):
                return "N"
            elif ((minprice > 0) & (maxprice > 0)) or ((minprice > 0) | (maxprice > 0)):
                return "Y"
        ...
        def fpinternal(x):
            ...
            intnotestr = f"FP:{intnoteval} +vivid"
        ...
        def bginternal(x):
            ...
            intnoteval = round(((minprice + fees) * 1.2), 2)
            if sh == "Y":
                intnotestr = f"BG:{intnoteval} -vivid -ticketevolution -tickpick -ticketnetwork -ticketmaster"
            else:
                intnotestr = (
                    f"BG:{intnoteval} -vivid -ticketevolution -tickpick -ticketnetwork -stubhub -gametime -ticketmaster"
                )
            if unbroadcast != "Unbroadcast":
                return intnotestr
            else:
